...
const IndCustHomePage: React.FC<Props> = ({ navigation, route }) => {
  ...
  const fetchTokenDetails = async () => {
    try {
      ...
      const custPrimaryKey = route.params?.custPrimaryKey;

      if (!custPrimaryKey) {
        throw new Error("Customer key not found. Please log in again.");
      }

      const db = getDatabase();
      const customerRef = ref(db, `CustomerRegistration/${custPrimaryKey}`);
      ...
      if (tokenSnapshot.exists()) {
        tokenSnapshot.forEach((childSnapshot) => {
          ...
          if (tokenData.email === customerEmail) {
  ...
  // Gas prices data remains the same
  const gasPrices = [
      ...
      <View style={styles.navigationBar}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate("IndCustHomePage", {
            custPrimaryKey: route.params?.custPrimaryKey
          })}>
        ...
        <TouchableOpacity
          style={styles.navItem}
          onPress={() => navigation.navigate("IndCustOrderHistory", {
            custPrimaryKey: route.params?.custPrimaryKey
          })}>
...
const styles = StyleSheet.create({
  ...
  headerText: { color: "#FFFFFF", fontSize: 26, fontWeight: "bold" },
  ...
  statusDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: "#2563EB", marginRight: 6 },
